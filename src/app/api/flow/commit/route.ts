...
async function processPayment(token: string, req: NextRequest) {
  ...
  try {
    ...
    if (order.status === 'PAID') {
      ...
      return NextResponse.redirect(new URL(`/ventasticker/exito?orden=${order.order_code}&tickets=${tks.rows.map((r:any)=>r.ticket_code).join(',')}`, req.url));
    }
    ...
    return NextResponse.redirect(new URL(`/ventasticker/exito?orden=${order.order_code}&tickets=${tickets.join(',')}`, req.url));
...
export async function GET(req: NextRequest) {
  ...
  const tbk_orden = req.nextUrl.searchParams.get('TBK_ORDEN_COMPRA')

  console.log('RETORNO GET:', { token_ws, tbk_token, tbk_orden })

  if (tbk_token) {
    console.log('TOKEN ANULADO CAPTURADO:', tbk_token)
    // NO hagas commit, solo marca la orden como abortada si quieres
    // await pool.query(`UPDATE orders SET status='ABORTED' WHERE order_code=$1`, [tbk_orden])
    return NextResponse.redirect(new URL(`/ventasticker?anulado=${tbk_token}`, req.url))
  }

  if (!token_ws) return NextResponse.redirect(new URL('/?error=no_token', req.url))
  return processPayment(token_ws, req)
}
export async function POST(req: NextRequest) {
  const form = await req.formData();
  const token_ws = form.get('token_ws') as string
  const tbk_token = form.get('TBK_TOKEN') as string
  const tbk_orden = form.get('TBK_ORDEN_COMPRA') as string

  console.log('RETORNO POST:', { token_ws, tbk_token, tbk_orden })
  ...
  if (!token_ws) return NextResponse.redirect(new URL('/?error=no_token_post', req.url))
  return processPayment(token_ws, req)
}