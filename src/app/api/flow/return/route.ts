...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#111111;border-radius:16px;overflow:hidden;border:1px solid #1f1f1f;">
  <tr><td style="background:#000000;padding:32px 24px 24px;text-align:center;border-bottom:3px solid #E30613;">
  ...
  <tr><td style="padding:32px 24px 8px;text-align:center;background:#111111;">
      <img src="https://www.balladares-motors.cl/escudo.png" style="height:64px;width:auto;display:block;margin:0 auto 16px;" />
      <div style="background:#FFD700;color:#000;font-family:Arial Black, Arial, sans-serif;display:inline-block;padding:8px 20px;border-radius:100px;font-weight:900;font-size:12px;">✓ PAGO APROBADO</div>
      <h1 style="font-family:Arial Black, Arial, sans-serif;color:#fff;margin:18px 0 8px;font-weight:900;font-size:28px;">¡Gracias por Participar!</h1>
      <p style="font-family:Arial, sans-serif;color:#a1a1aa;font-size:14px;margin:0;">Tu pago se confirmó.</p>
    </td></tr>
  <tr><td style="padding:20px 24px;background:#111111;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#1a1a1a;border:1px solid #262626;border-radius:12px;"><tr><td style="padding:16px 18px;">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr><td style="font-family:Arial, sans-serif;color:#71717a;font-size:11px;font-weight:700;padding-bottom:6px;">ORDEN</td><td style="font-family:Arial, sans-serif;color:#71717a;font-size:11px;font-weight:700;text-align:right;padding-bottom:6px;">FECHA</td></tr>
              ...
              <tr><td style="padding-top:12px;font-family:Arial, sans-serif;color:#a1a1aa;font-size:13px;">Pack x${qty} • ${tickets.length} ticket(s)</td><td style="padding-top:12px;font-family:Arial, sans-serif;color:#FFD700;font-size:13px;font-weight:900;text-align:right;">PAGADO - Webpay</td></tr>
              <tr><td colspan="2" style="padding-top:6px;font-family:Arial, sans-serif;color:#71717a;font-size:12px;">Enviado a: <span style="color:#60a5fa;">${email}</span></td></tr>
            </table>
      </td></tr></table>
    </td></tr>
  <tr><td style="padding:8px 24px 8px;background:#111111;">
      ...
      ${tickets.map((t,i) => `<table width="100%" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:10px;margin-bottom:10px;overflow:hidden;"><tr><td style="width:6px;background:#E30613;"></td><td style="padding:16px 18px;"><div style="font-family:Arial, sans-serif;color:#E30613;font-size:11px;font-weight:900;">TICKET #${i+1}</div><div style="font-family:Courier New, monospace;color:#000;font-size:20px;font-weight:900;letter-spacing:3px;margin-top:2px;">${t}</div></td><td style="padding:16px 18px;text-align:right;"><div style="background:#000;color:#FFD700;font-family:Arial, sans-serif;font-size:10px;font-weight:900;padding:8px 12px;border-radius:6px;display:inline-block;">VÁLIDO</div></td></tr></table>`).join('')}
    </td></tr>
  <tr><td style="padding:0 24px 16px;background:#111111;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background:linear-gradient(135deg,#1a1a1a,#222);border:1px solid #FFD700;border-radius:12px;"><tr><td style="padding:20px;text-align:center;">
            <div style="font-family:Arial Black, Arial, sans-serif;color:#FFD700;font-size:14px;font-weight:900;margin-bottom:6px;">🎁 REGALO EXCLUSIVO</div>
            ...
            <a href="https://www.balladares-motors.cl/stickers-pack.zip" target="_blank" style="background:#FFD700;color:#000;font-family:Arial Black, Arial, sans-serif;font-size:13px;font-weight:900;padding:14px 28px;border-radius:100px;text-decoration:none;display:inline-block;">⬇ DESCARGAR STICKERS.ZIP</a>
      </td></tr></table>
    </td></tr>
  <tr><td style="padding:20px 24px;text-align:center;background:#000;border-top:1px solid #1f1f1f;">
      <p style="font-family:Arial, sans-serif;color:#52525b;font-size:11px;margin:0;">¿Dudas? <a href="mailto:hola@balladares-motors.cl" style="color:#fff;text-decoration:none;font-weight:700;">hola@balladares-motors.cl</a></p>
    </td></tr>
</table>
</td></tr>
</table>
...
async function processOrder(rawToken: string, orderFromUrl: string) {
  ...
  if (process.env.FLOW_ENV !== 'sandbox') {
    const r = await fetch(`${FLOW_API_URL}/payment/getStatus`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body
    })
    const data = await r.json()
    console.log('FLOW STATUS RETURN FIX', data, 'TOKEN', token.slice(0,15))
    if (data.status !== 2) return { orderCode: data.commerceOrder || orderFromUrl, paid: false }
    orderFromUrl = data.commerceOrder
  }

  const client = await pool.connect()
  try {
    await client.query('BEGIN')
    const { rows } = await client.query(`SELECT * FROM orders WHERE order_code=$1 FOR UPDATE`, [orderFromUrl])
    const order = rows[0]
    if (!order) { await client.query('ROLLBACK'); return { orderCode: orderFromUrl, paid: false } }
