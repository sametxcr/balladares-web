...
export async function POST(req: NextRequest) {
  ...
  const amount = pack_id === 'x1' ? 3000*(pack_qty||1) : 10000*(pack_qty||1);
  const qty = pack_id === 'x1' ? (pack_qty||1) : (pack_qty||1)*4;
  ...
  const tbkRes = await fetch('https://webpay3gint.transbank.cl/rswebpaytransaction/api/webpay/v1.2/transactions',{
    ...
    body: JSON.stringify({ buy_order: order_code, session_id: `S${Date.now()}`, amount, return_url: 'https://www.balladares-motors.cl/api/webpay/commit' })
  });
