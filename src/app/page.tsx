...
const slides = [
  ...
  { title: ["diagnostico de", "ultima generacion"], titleImgs: ["/hero/titles/diagnostico_de_transparent.png", "/hero/titles/ultima_generacion_transparent.png"], sub: "Scanner multimarca", img: "/hero/scanner.jpg", pos: "50% 50%" },
];

const servicios = [
  { n: "Repro Stage 1/2", p: "Desde $180.000", d: "Libera el verdadero potencial oculto de tu ECU.", icon: "/icons/turbochip_isometric_icon.webp", accent: "from-red-600 to-orange-500", badge: "POPULAR" },
  { n: "Scanner Multimarca", p: "Desde $25.000", d: "Diagnóstico profundo.", icon: "/icons/obd2_scanner_icon.webp", accent: "from-blue-600 to-cyan-400", badge: "DIAGNOSIS" },
  { n: "Alineación 3D", p: "Desde $18.000", d: "Precisión milimétrica láser 3D.", icon: "/icons/wheel_alignment_icon.webp", accent: "from-zinc-600 to-zinc-400", badge: "LASER" },
  { n: "Balanceo", p: "Desde $12.000", d: "Balanceo dinámico.", icon: "/icons/racing_wheel_balancer_icon.webp", accent: "from-yellow-500 to-amber-500", badge: "0 VIBRACIÓN" },
  { n: "Ajuste Motor", p: "Cotizar", d: "Armado y rectificación alto rendimiento.", icon: "/icons/balladares_motors_emblem_transparent.webp", accent: "from-red-600 to-red-800", badge: "AJUSTE" },
  { n: "Mecánica General", p: "Cotizar", d: "Frenos, suspensión, distribución.", icon: "/icons/sr20_neo_vvl_balladares.webp", accent: "from-zinc-700 to-zinc-500", badge: "MECANICA GENERAL" },
  { n: "Venta Etanol / Metanol R117", p: "Cotizar", d: "Combustible competición.", icon: "/icons/racing_fuel_canister_icon.webp", accent: "from-orange-600 to-red-600", badge: "RACING FUEL" },
];
          ...
          <div key={idx} className={`absolute inset-0 transition-opacity duration-1000 ${idx===i?"opacity-100":"opacity-0"}`}>
            ...
            <div className="relative h-full flex flex-col justify-between px-4 md:px-24 py-10 md:py-20">
              ...
              <div className="flex flex-col gap-3 items-start mb-2">
                <a href="#servicios" className="w-fit bg-red-600 px-8 py-3.5 font-black text-" style={{transform:"skewX(-12deg)"}}><span style={{transform:"skewX(12deg)", display:"block"}}>VER SERVICIOS →</span></a>
                <div className="bg-white text-black inline-flex px-5 py-2.5 font-black text- w-fit shadow-[4px_4px_0px_#dc2626]" style={{transform:"skewX(-12deg)"}}><span style={{transform:"skewX(12deg)", display:"block"}}>{s.sub}</span></div>
              </div>
            </div>
          </div>
        ))}
      ...
      <section id="servicios" className="bg-white text-black px-4 md:px-24 py-16 md:py-20">
        ...
        <div className="grid md:grid-cols-3 gap-6 md:gap-8 mt-8">
          {servicios.map(s=>{
            ...
            return (
              <div key={s.n} onMouseEnter={()=>setActiveService(s.n)} onMouseLeave={()=>setActiveService(null)} className="group relative cursor-pointer">
                ...
                <div className="relative bg-white p-" style={{transform:"skewX(-3deg)"}}>
                  <div className="bg-white" style={{transform:"skewX(3deg)"}}>
                    <div className="relative h-64 md:h-72 bg-gradient-to-br from-zinc-50 to-zinc-100 flex items-center justify-center overflow-hidden border-b border-black/5">
                      <img src={s.icon} alt={s.n} className="w-[90%] h-[90%] object-contain" />
                      <div className="absolute top-3 right-3 bg-black text-white text- font-black px-3 py-1.5">{s.badge}</div>
                    ...
                    <div className="p-5">
                      <div className="font-black text- italic leading-tight">{s.n.toUpperCase()}</div>
                      <div className="mt-2 text-red-600 font-black text-">{s.p}</div>
                      <a href={`https://wa.me/${WHATSAPP}?text=Hola, quiero cotizar ${encodeURIComponent(s.n)}`} target="_blank" className="mt-4 inline-flex w-full justify-center bg-black text-white py-3 text-sm font-black group-hover:bg-red-600 transition">COTIZAR →</a>
                    </div>
                  </div>
                </div>
              </div>
