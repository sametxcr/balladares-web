...
const servicios = [
  { 
    n: "Repro Stage 1/2", 
    p: "Desde $180.000", 
    d: "Libera el verdadero potencial oculto de tu ECU. Ganancia real de +25% HP y +30% torque sin comprometer fiabilidad. Mapeos a medida para calle y pista.",
    icon: "/icons/turbochip_isometric_icon.webp",
    accent: "from-red-600 to-orange-500",
    badge: "POPULAR"
  },
  { 
    n: "Scanner Multimarca", 
    p: "Desde $25.000", 
    d: "Diagnóstico profundo con equipamiento de última generación. Lectura de fallas, datos en vivo y reseteo de servicios para todas las marcas europeas, japonesas y americanas.",
    icon: "/icons/obd2_scanner_icon.webp",
  ...
  { 
    n: "Alineación 3D", 
    p: "Desde $18.000", 
    d: "Precisión milimétrica con sistema láser 3D. Corrige desgaste irregular, mejora agarre en curva y devuelve la estabilidad a alta velocidad. Setup de pista disponible.",
    icon: "/icons/wheel_alignment_icon.webp",
  ...
  { 
    n: "Balanceo", 
    p: "Desde $12.000", 
    d: "Adiós vibraciones. Balanceo dinámico computarizado para llantas hasta aro 22. Ideal para autos preparados que superan los 200 km/h sin drama.",
    icon: "/icons/racing_wheel_balancer_icon.webp",
  ...
  { 
    n: "Ajuste Motor", 
    p: "Cotizar", 
    d: "Armado y rectificación de motores de alto rendimiento. Desde empaquetaduras a forjado completo. Experiencia en pista, no aprendemos con tu auto.",
    icon: "/icons/balladares_motors_emblem_transparent.webp",
  ...
  { 
    n: "Mecánica General", 
    p: "Cotizar", 
    d: "Frenos, suspensión, distribución, embrague y tren delantero. Especialistas en Nissan SR20 NEO VVL, con tapa de válvulas personalizada Balladares Motors. Mano de obra que entiende de potencia real.",
    icon: "/icons/sr20_neo_vvl_balladares.webp",
...
export default function Page(){
  ...
  return (
    <main className="bg-black text-white overflow-x-hidden">
      ...
      {/* HERO - LETRAS BLANCAS BAJADAS UN POCO Y SEPARADAS, SIEMPRE A LA IZQUIERDA */}
        ...
        {slides.map((s,idx)=>(
          <div key={idx} className={\`absolute inset-0 transition-opacity duration-1000 \${idx===i?"opacity-100":"opacity-0"}\`}>
            ...
            <div className="relative h-full flex flex-col justify-between px-4 md:px-24 py-10 md:py-20">
              {/* TITULO - BAJADO Y SEPARADO */}
              <div className="mt-16 md:mt-28">
                <h1 className="text-[40px] md:text-[84px] font-black italic leading-[0.92] md:leading-[0.95] max-w-5xl tracking-[-0.02em] text-left drop-shadow-[3px_3px_0px_rgba(0,0,0,0.8)]">
                  {s.title.split(" ").map((word, wi) => (
                    <span key={wi} className="block">{word}{" "}</span>
                  ))}
                </h1>
                {/* Si el titulo es largo de 2 lineas, lo separamos mas */}
                <style>{\`h1 span + span { margin-top: 0.08em; }\`}</style>
              </div>
              {/* BOTONES ABAJO A LA IZQUIERDA */}
              <div className="flex flex-col gap-3 items-start mb-2 md:mb-2">
      ...
      <section id="galeria" className="bg-[#0a0a0a] px-4 md:px-24 py-16 border-y border-white/10">
        ...
        <button onClick={()=>setShowAll(!showAll)} className="mt-6 bg-white text-black px-6 py-2 font-black text-sm hover:bg-red-600 hover:text-white transition mx-auto block">{showAll ? "VER MENOS" : \`VER LAS \${galeriaTaller.length} FOTOS →\`}</button>
      </section>
