...
const slides = [
  ...
  { title: "SERVICIO DE PISTA Y CALLE", sub: "Alineación 3D, balanceo, elevadores", img: "/hero/pista.jpg", pos: "50% 20%", scale: 0.7 },
];
...
export default function Page(){
  ...
  return (
    <main className="bg-black text-white overflow-x-hidden">
        ...
        {slides.map((s,idx)=>(
          <div key={idx} style={{position:"absolute", inset:0, opacity: idx===i?1:0, transition:"opacity 1s"}}>
            ...
            <div style={{position:"relative", height:"100%", display:"flex", flexDirection:"column", justifyContent:"center", padding:"0 80px"}}>
              <h1 style={{fontSize:"80px", fontWeight:900, fontStyle:"italic", lineHeight:0.9, maxWidth:700}}>{s.title}</h1>
              <p style={{marginTop:16, background:"white", color:"black", display:"inline-block", padding:"6px 16px", fontWeight:700, width:"fit-content", transform:"skewX(-12deg)"}}>
                <span style={{transform:"skewX(12deg)", display:"block"}}>{s.sub}</span>
              </p>
              <a href="#servicios" style={{marginTop:24, width:"fit-content", background:"#dc2626", padding:"12px 32px", fontWeight:900, transform:"skewX(-12deg)", display:"inline-block"}}>
      ...
      <section id="servicios" style={{background:"white", color:"black", padding:"80px 96px"}}>
        ...
        <div style={{display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:24, marginTop:40}}>
          {servicios.map(s=><div key={s.n} style={{border:"2px solid black", padding:24}}>
            ...
            <a href={`https://wa.me/${WHATSAPP}?text=Hola, quiero cotizar ${encodeURIComponent(s.n)}`} target="_blank" style={{marginTop:20, display:"inline-block", background:"#dc2626", color:"white", padding:"8px 20px", fontSize:14, fontWeight:900}}>COTIZAR</a>
          </div>)}
        </div>
      </section>
      ...
      <a href={`https://wa.me/${WHATSAPP}`} target="_blank" style={{position:"fixed", bottom:24, right:24, background:"#25D366", width:56, height:56, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:24, fontWeight:900, boxShadow:"0 10px 20px rgba(0,0,0,0.3)"}}>W</a>
    </main>
  );
}
