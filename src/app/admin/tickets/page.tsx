...
export default async function Page({ searchParams }: { searchParams: Promise<{ q?: string, key?: string, page?: string }> }) {
  ...
  if (key!== process.env.ADMIN_SECRET) {
    return <div className="min-h-screen bg-black text-white grid place-items-center font-black tracking-widest">🔒 NO AUTORIZADO</div>
  }
        ...
        <div className="bg-[#101010] border border-zinc-900 rounded- overflow-hidden">
          ...
          <div className="flex justify-between items-center p-3 border-t border-zinc-900 bg-black">
            ...
            <div className="flex gap-2">
              {page>1 && <Link href={`/admin/tickets?key=${key}&q=${q}&page=${page-1}`} className="px-4 h-8 grid place-items-center bg-zinc-900 border border-zinc-800 rounded-full text- font-black hover:bg-zinc-800">← ANTERIOR</Link>}
              {page<totalPages && <Link href={`/admin/tickets?key=${key}&q=${q}&page=${page+1}`} className="px-5 h-8 grid place-items-center bg-white text-black rounded-full text- font-black hover:bg-zinc-200">SIGUIENTE →</Link>}
            </div>
          </div>
        </div>
      </div>
